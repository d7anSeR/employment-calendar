...
public class EmployeeController {
    ...
    // Проверка пользователя (аутентификация)
    ...
    @PutMapping("/employee")
    public ApiResponse<Employee> addOrUpdateEmployee(@RequestBody EmployeeRequest request) {
        try {
            Employee employee = service.createOrUpdateEmployee(request);
            return new ApiResponse<>(true, "Пользователь создан или обновлен", employee);
        } catch (Exception e) {
